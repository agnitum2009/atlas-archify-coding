...
import { parseLocator, anchorState, anchorQuality, checkVerifiedHeadAnchors } from './evidence.mjs';
...
// 阈值参数化（2026-09-11，编排线技术自决并留痕）：<侧车同目录>/ledger-size.json 可覆盖默认阈值
// （形状 { schemaVersion, maxBytes?, maxTraces?, rationale? }）。无文件/坏文件 = 默认值（零破坏）。
//
// 立法依据（实测 2026-09-11，demo-b 侧车 1.73MB）：read 4.35ms + parse 2.77ms + write 1.68ms 约 8.8ms/轮，
// 成本随体积**线性**（JSON parse/stringify 无超线性拐点）⇒ 1MB 是**保守启发式、非性能拐点**；
// 且 demo-b 侧车 nodes 段占约 49%（850KB）属**不可归档**（拆文件会破坏单一侧车 CAS 语义），
// 清空全部已读 notices + trace 后仍约 1.29MB ⇒ 单靠归档达不到 1MB。故阈值做成可配置，
// 由项目按实测定档并写明 rationale（留痕落点），而非把不可达阈值长期挂成常红警告。
export function ledgerSizeLimits(sidecarPath) {
  const res = { maxBytes: LEDGER_SIZE_BYTES, maxTraces: LEDGER_SIZE_TRACES, source: 'default', rationale: null };
  if (!sidecarPath) return res;
  let cfg = null;
  try { cfg = JSON.parse(fs.readFileSync(path.join(path.dirname(sidecarPath), 'ledger-size.json'), 'utf8')); } catch { cfg = null; }
  if (!cfg || typeof cfg !== 'object') return res;
  const mb = Number(cfg.maxBytes);
  if (Number.isFinite(mb) && mb > 0) { res.maxBytes = mb; res.source = 'config'; }
  const mt = Number(cfg.maxTraces);
  if (Number.isFinite(mt) && mt > 0) { res.maxTraces = mt; res.source = 'config'; }
  if (typeof cfg.rationale === 'string') res.rationale = cfg.rationale;
  return res;
}
...
export function runDoctor(opts) {
  ...
  // 2026-09-14 回归修复（e-contract recon 抓获）：NODE_SAMPLE / nodeSample 原声明在下方 if 块内，
  // 而 cancelled-evidence 检查在 if 块外也引用二者 → ReferenceError，doctor 对任何有节点的侧车整体 failed。
  // 二者本属函数级复用（evidence-resolvability 与 cancelled-evidence 同式），故上提到函数作用域。
  const NODE_SAMPLE = 5;
  ...
  const limits = ledgerSizeLimits(o.sidecar);
  const ledgerOversize = sidecar === 'ok' && (sidecarBytes > limits.maxBytes || traceCount > limits.maxTraces);
  checks.push({
    name: 'ledger-size',
    ...
    detail: sidecar === 'ok'
      ? (ledgerOversize
          ? '侧车 ' + sidecarBytes + ' 字节 / trace ' + traceCount + ' 条（阈值 ' + limits.maxBytes + ' 字节 / ' + limits.maxTraces + ' 条，来源=' + limits.source + '）；考虑冷归档到 history/ 区（仅提示，不自动动账）'
          : '侧车 ' + sidecarBytes + ' 字节 / trace ' + traceCount + ' 条（阈值 ' + limits.maxBytes + ' 字节 / ' + limits.maxTraces + ' 条，来源=' + limits.source + '）')
      : 'sidecar 不可读，账本规模未检',
  });

  // A3-cancelled 完整性（2026-09-14）：progress=cancelled 但零证据的节点=「无依据取消」——
  // 与证据可解析性同理，属数据债 warning 不阻断，但在操作面可闻（S 系列误取消教训：
  // 取消是终态声明，须锚定被取代/退役依据）。列表体例沿用 nodeSample（前 5 逐条+全量汇总）。
  const cancelledNoEvidence = [];
  const cancelledNoEvidenceSet = new Set();
  if (sidecar === 'ok') {
    for (const [id, node] of Object.entries(sidecarData.nodes || {})) {
      if (!node || node.progress !== 'cancelled') continue;
      if (Array.isArray(node.evidence) && node.evidence.length > 0) continue;
      cancelledNoEvidenceSet.add(id);
      if (cancelledNoEvidence.length < NODE_SAMPLE && !cancelledNoEvidence.includes(id)) cancelledNoEvidence.push(id);
    }
  }
  checks.push({
    name: 'cancelled-evidence',
    ok: cancelledNoEvidenceSet.size === 0,
    warning: true,
    detail: sidecar === 'ok'
      ? (cancelledNoEvidenceSet.size === 0
          ? '无 cancelled 节点缺证据（A3-cancelled 守卫已生效：取消须先 evidence-add）'
          : 'cancelled 节点 ' + cancelledNoEvidenceSet.size + ' 个零证据（无依据取消，须补 evidence-add 锚定被取代/退役依据；' + nodeSample(cancelledNoEvidence, cancelledNoEvidenceSet) + '）')
      : 'sidecar 不可读，cancelled 证据未检',
  });

  // O2（2026-09-14 设计件 §2 O2）：锚对 HEAD 校验（warning 级数据债，与 evidence-resolvability 同纪律）
  // ——progress=verified 节点的锚在 git HEAD 里必须存在且行内容与工作树一致（以 git show HEAD:<rel> 比对）。
  // 工作树-only 锚（未提交即当证据）/内容不一致 = error 级发现但只作 doctor warning（医生不阻断开工）；
  // gitignored（atlas artifacts/ 类生成物）单列计数、无 git 仓记 noGit 不误报。
  let evidenceHead = null;
  if (sidecar === 'ok') {
    const head = checkVerifiedHeadAnchors(sidecarData.nodes || {}, process.cwd());
    evidenceHead = {
      verdict: head.verdict,
      ok: head.ok,
      checkedNodes: head.checkedNodes,
      checkedAnchors: head.checkedAnchors,
      checked: head.checked,
      exempt: head.exempt,
      unchecked: head.unchecked,
      counts: head.counts,
      uncommitted: head.uncommitted.count,
      contentMismatch: head.contentMismatch.count,
      gitignored: head.gitignored.count,
      noGit: head.noGit.count,
      noGitReasons: head.noGitReasons,
      samples: {
        uncommitted: head.uncommitted.sample,
        contentMismatch: head.contentMismatch.sample,
        gitignored: head.gitignored.sample,
        noGit: head.noGit.sample,
      },
    };
    // 2026-09-15（缺陷3）：显示口径与 report 同源（verdict 由 lib/evidence.mjs 的同一聚合给出）——
    // 「免检/未检查/零检查」不得被读成「已一致」：先逐项给数（真比对 / 免检 / 未检查），再给结论。
    const verdictText = {
      verified: '工作树行内容 == git show HEAD:<rel>',
      partial: '本轮比对不完整（部分锚免检或未检查）',
      'exempt-only': '本轮锚全部免检（gitignored/no-git），未取得一致性证据',
      'unchecked-only': '本轮锚全部不可比对（broken/unparseable/timeout），未取得一致性证据',
      'no-scope': '无 progress=verified 节点或无锚——未作 HEAD 一致性断言（无对象可验 ≠ 已验证）',
      inconsistent: '发现工作树 ≠ HEAD 的锚',
    }[head.verdict] || '（未知判决）';
    const parts = [
      'verdict=' + head.verdict + '：' + verdictText,
      '真比对通过 ' + head.checked + ' 条 · 免检 ' + (head.exempt.gitignored + head.exempt.noGit) +
        ' 条（gitignored ' + head.exempt.gitignored + '、no-git ' + head.exempt.noGit + '）· 未检查 ' +
        (head.unchecked.broken + head.unchecked.unparseable + head.unchecked.timeout) + ' 条（broken ' + head.unchecked.broken + '、unparseable ' + head.unchecked.unparseable + '、timeout ' + head.unchecked.timeout + '）',
      'progress=verified 节点 ' + head.checkedNodes + ' 个 / 锚 ' + head.checkedAnchors + ' 条',
    ];
    for (const d of head.errors.slice(0, 5)) parts.push(d.rule + '：' + d.evidence);
    if (head.errors.length > 5) parts.push('…另 ' + (head.errors.length - 5) + ' 条 error');
    checks.push({ name: 'head-anchor-consistency', ok: head.ok, warning: true, detail: parts.join('；') });
  } else {
    checks.push({ name: 'head-anchor-consistency', ok: false, warning: true, detail: 'sidecar 不可读，锚对 HEAD 未检' });
  }
  ...
  if (evidenceHead) result.evidenceHead = evidenceHead;
