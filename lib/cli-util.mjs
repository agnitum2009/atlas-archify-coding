...
export function diag(rule, message, subject, severity = 'error') {
...
export function autoTrace(command, args, entry, preloaded) {
  ...
  if (!preloaded && !args.sidecar) return null;
  try {
    // 路径解析放在 try 内：断链 symlink / EACCES 等不可解析侧车同样只降级，不让主结果变 exit 1。
