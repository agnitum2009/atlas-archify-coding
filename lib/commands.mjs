...
const COMMANDS = [
  ...
  },
  {
    ...
    usage: [
      ...
      '  set/transition：truth 轴前进写入必填 --receipt <负责人本地回执文件>（缺失/null 初态按 candidate；机器只校验普通文件，不校验语义）；set 对已存在节点轴值变更同过 A2 迁移表（2026-08-15 裁定④，不再架空），初始化/该轴首写免表，违表=exit 1 illegal_transition，确属纠错加 --correction（实际豁免规则时 history 事件 corrected:true 留痕；不免除 truth 回执与证据）；证据守卫：verified 与 truth effective/closed 要求非空可解析锚，cancelled 要求非空（含首次写）、set ledger→settled=settled_requires_event（只能经 settle/import 事件；同值原地写不拦；--correction 纠错通道保留）、progress→cancelled 而 ledger≠clean=cancelled_requires_clean（0.21.1：欠账取消成永久孤儿，先 settle 核销或 --correction 显式核销留痕）；仅 set 可初始化缺失账本',
    ],
    run: runState,
  },
  {
