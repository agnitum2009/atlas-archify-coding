...
function runState(argv) {
  ...
  try {
    ...
    if (e.code === 'sidecar_missing' && sub === 'set') {
  ...
  try {
    ...
    if (sub === 'transition') {
      ...
      if (node[args.axis] !== args.from) {
        printAndExit(failed(cmd, [diag('transition_from_mismatch', 'transition 的 from 必须等于节点当前 ' + args.axis + ' 状态：当前为 ' + node[args.axis] + '，收到 ' + args.from, args.node)]), 1);
        return;
      }
