...
const FLAGS = {
  ...
  state: ['node', 'axis', 'value', 'reason', 'owner', 'receipt', 'correction', 'sidecar', 'locator', 'from', 'to', 'with-backlog', 'kind', 'all', 'ref', 'remove', 'source', 'cutoff'],
...
import { isAxis, isValidState, validateTransition, validateSetWrite, ACTIVE_CLASSES } from './state-machine.mjs';
...
function runState(argv) {
  ...
  const gate = (sub === 'set' || sub === 'transition' || sub === 'settle' || sub === 'block' || sub === 'import') ? loadProjectGate(path) : null;
  ...
  try {
    if (sub === 'spec-ref') {
      ...
      const isRemove = args.remove === true || args.remove === 'true';
      if (isRemove) {
    ...
    if (sub === 'active') {
      ...
      printAndExit(ok(cmd, {
        ...
        criterion: 'class ∈ 活帐类 且 progress ∉ {verified, cancelled}',
        nodes: shown.sort((a, b) => a.id.localeCompare(b.id)),
        ...(all ? {} : { note: '默认只列活帐类（task/debt/batch-gated/trigger-gated）且未完成；--all 列全部含未分类' }),
      }), 0);
      return;
    }
    ...
    if (sub === 'get') {
      ...
      printAndExit(ok(cmd, {
        node: args.node, owner: node.owner, truth: node.truth, progress: node.progress,
    ...
    if (sub === 'set') {
      ...
      // A3 守卫（0.17.0，路线一裁定）：progress 写到 verified 必须至少 1 条 Evidence——与 transition/settle 同构，
      // init 首写不再豁免（终态声称必须证据绑定：Init ⇒ Inv，不变量对初始化同为全称）；历史导入请用 state import。
      // --correction 显式纠错通道放行（corrected:true 留痕）；同值原地写（before===value，无状态变更）不拦。
      const verifiedGuardHit =
        args.axis === 'progress' &&
        args.value === 'verified' &&
        before !== 'verified' &&
        (node.evidence || []).length < 1;
      if (verifiedGuardHit && !args.correction) {
        printAndExit(failed(cmd, [diag('verified_requires_evidence', 'A3：progress 写到 verified 必须至少 1 条 Evidence（先 state evidence-add；历史/迁移导入用 state import）', args.node)], { lessonPrompt: LESSON_PROMPT }), 1);
        return;
      }
      // A2 §2.4 守卫（0.17.0，路线一裁定）：ledger→settled 只能经跨轴事件写入——state settle（执行闭环）
      // 或 state import（历史导入）；set 直达（含 init 首写）会绕过「双写同回执」不变量，一律拒。
      // --correction 为唯一显式出口（corrected:true 留痕）；同值原地写不拦。
      const settledGuardHit = args.axis === 'ledger' && args.value === 'settled' && before !== 'settled';
      if (settledGuardHit && !args.correction) {
        printAndExit(failed(cmd, [diag('settled_requires_event', 'A2 §2.4：ledger→settled 只能经 state settle（执行闭环）或 state import（历史导入）事件写入——set 直达会破坏跨轴双写不变量', args.node)], { lessonPrompt: LESSON_PROMPT }), 1);
        return;
      }
      // A3-cancelled 守卫（2026-09-14）：progress 迁到 cancelled 是终态声明（退役/取代），
      // 与 verified 同构须先 evidence-add——防止无依据取消关键节点（S 系列误取消教训）。
      // --correction 显式纠错通道放行（与 A2 一致），history 带 corrected:true 留痕。
      if (
        args.axis === 'progress' &&
        args.value === 'cancelled' &&
        nodeExisted &&
        !args.correction &&
        (node.evidence || []).length < 1
      ) {
        printAndExit(failed(cmd, [diag('cancelled_requires_evidence', 'A3：progress 迁到 cancelled 必须至少 1 条 Evidence（先 state evidence-add——取消是终态声明，须锚定被取代/退役依据）', args.node)], { lessonPrompt: LESSON_PROMPT }), 1);
        return;
      }
      ...
      if (a2.admittedCorrection || ((verifiedGuardHit || settledGuardHit) && args.correction)) {
        event.corrected = true; // 纠错通道留痕：真实绕过 A2 表或 0.17.0 终态守卫的写入才打标（守卫命中但表合法时 admittedCorrection 不覆盖）
      }
    ...
    if (sub === 'transition') {
      ...
      // A3-cancelled 守卫（2026-09-14）：progress 迁到 cancelled 是终态声明，须 evidence-add。
      if (args.axis === 'progress' && args.to === 'cancelled' && (node.evidence || []).length < 1) {
        printAndExit(failed(cmd, [diag('cancelled_requires_evidence', 'A3：progress 迁移到 cancelled 必须至少 1 条 Evidence（先 state evidence-add——取消是终态声明，须锚定被取代/退役依据）', args.node)], { lessonPrompt: LESSON_PROMPT }), 1);
        return;
      }
    ...
    if (sub === 'settle') {
      ...
      const graphBinding = bindingStatusFor(args.node, node.specRefs, path);
    ...
    if (sub === 'import') {
      ...
      const node = ensureNode(sidecar, args.node, args.owner);
      node.evidence = node.evidence || [];
      if (!node.evidence.includes(abs.locator)) node.evidence.push(abs.locator);
      ...
      node.progress = 'verified';
      node.ledger = 'settled';
      const event = { at: new Date().toISOString(), kind: 'import', from: before, to: { progress: 'verified', ledger: 'settled' }, reason: args.reason, by: args.owner, locator: abs.locator };
      if (args.source !== undefined) event.source = String(args.source);
      if (args.cutoff !== undefined) event.cutoff = String(args.cutoff);
      appendHistory(node, event);
      ...
      saveSidecar(path, sidecar);
      const next = '销账五动作第4步：atlas-engine report --sidecar ' + path + ' 生成销账回执';
      printAndExit(ok(cmd, { node: args.node, from: before, to: { progress: 'verified', ledger: 'settled' }, next, lessonPrompt: LESSON_PROMPT, receipt: { rule: 'A2-cross-axis-import', status: 'ok', dualWrite: true, provenance: 'imported' } }), 0);
      return;
    }
...
function runReport(argv) {
  ...
  try {
    ...
    const report = buildReport(sidecar, { slice: args.slice || null, root: args.root || '.', verify, codeSha: args['code-sha'] || null, specSha: args['spec-sha'] || null, specs, replays: args.replay === undefined ? [] : [].concat(args.replay), brief: !!args.brief, nonAccountsPath });
...
const COMMANDS = [
  ...
  },
  {
    ...
    usage: [
      ...
      'atlas-engine state spec-ref --node <id> --ref <图件id> [--remove] [--sidecar <path>]（A3：显式认领图件 id——认领后 A1 不再报图件未入账）',
      'atlas-engine state active [--all] [--sidecar <path>]（活帐视图：默认只列 class ∈ task|debt|batch-gated|trigger-gated）',
      'atlas-engine state get --node <id> [--sidecar <path>]',
      'atlas-engine state set --node <id> --axis truth|progress|ledger|class --value <v> --reason <text> --owner <o> [--receipt <回执文件>] [--correction] [--kind meta] [--sidecar <path>]（class=账务分类：declared|registry|container|task|debt|batch-gated|trigger-gated）',
      ...
      'atlas-engine state import --node <id> --reason <text> --owner <o> --locator <文件:行号> [--source <来源系统>] [--cutoff <截止日期>] [--sidecar <path>]（0.17.0：历史/迁移导入的唯一合法跨轴写——原子双写 verified+settled，强制证据锚，history kind=import 带溯源；只登记新节点或零执行史节点）',
      'atlas-engine state block --node <id> --reason <text> --owner <o> [--with-backlog] [--sidecar <path>]',
      '  set/transition：truth 轴前进写入必填 --receipt <负责人本地回执文件>（机器只校验存在性不校验语义，提案③ 2026-08-15）；set 对已存在节点轴值变更同过 A2 迁移表（2026-08-15 裁定④，不再架空），初始化/该轴首写免表，违表=exit 1 illegal_transition，确属纠错加 --correction（history 事件 corrected:true 留痕；不免除 truth 回执）；0.17.0 终态守卫：set progress→verified 无证据=verified_requires_evidence（init 首写不豁免）、set ledger→settled=settled_requires_event（只能经 settle/import 事件；同值原地写不拦；--correction 放行）',
    ],
    run: runState,
  },
  {
