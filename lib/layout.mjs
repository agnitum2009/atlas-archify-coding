...
function shaIsCommit(gitRoot, sha) {
  const res = gitSync(['cat-file', '-e', sha + '^{commit}'], { cwd: gitRoot });
...
export function validateLayout(rootDir, opts) {
  ...
  const registry = readProjectsRegistry(path.join(root, 'state', 'projects.json'));
  ...
  for (const entry of registry.entries) {
    if (entry && typeof entry.umbrella === 'string' && typeof entry.project === 'string') {
      if (registryByUmbrella.has(entry.umbrella) && registryByUmbrella.get(entry.umbrella) !== entry.project) {
        d.push(diag('layout.registry', 'warning', 'state/projects.json', '伞映射到多个项目：' + entry.umbrella));
        registryByUmbrella.set(entry.umbrella, null);
      } else registryByUmbrella.set(entry.umbrella, entry.project);
    }
  }
  for (const e of entries) {
    if (e.isFile() && e.name === 'INDEX.md') continue;
    if (e.isFile() && e.name === 'index.html') continue; // §〇.1 根可视化索引（build-portal --root 纯生成物）
    // 2026-09-11（e-contract 全盘扫描 G6）：数据根本身纳入 git 版本控制（§.gitignore 头注「负责人『所有改动须可 git 回滚』令」）
    // 后，.git/.gitignore/.gitattributes 是版本控制必需件，不再是「平铺违规」——显式放行（原仅因点文件降级 warning，仍属噪音）。
    if (e.name === '.git' || e.name === '.gitignore' || e.name === '.gitattributes') continue;
    ...
    if (e.isDirectory() && UMBRELLA_RE.test(e.name)) {
      ...
      if (regProject === undefined) {
        const fallback = projects.filter((p) => stripped === p || stripped.startsWith(p + '-')).sort((a,b) => b.length - a.length || a.localeCompare(b))[0];
        if (!fallback) {
      ...
      } else if (!projects.includes(regProject)) {
  ...
  for (const csvPath of csvFiles) {
    ...
    for (let i = 1; i < lines.length; i += 1) {
      ...
      if (cell === '') {
        ...
        shaRows.push({ at, sha: gitSha[1], project: path.basename(path.dirname(csvPath)) }); // 格式合法即 compliant；存在性留给二层校验统一判
      } else if (!/:\d+/.test(cell)) {
  ...
  // 跨仓证据（2026-09-11，e-contract 全盘扫描 G6）：data/<项目>/ 的提交级证据可能属于该项目**源仓**
  // （state/projects.json 的 sourcePath），而非图谱数据根自身——实证 data/add/add-command-progress.csv
  // 记录的是 atlas-engine 命令开发提交，根仓（demo-ledger）恒 miss，9 条 p5-sha-broken 全属误报面。
  // 处置：根仓未命中时回退该项目源仓解析；二处皆缺才报 broken（不伪报、不静默）。源仓未登记/路径不存在
  // 则退回原行为（只有根仓一个候选）。根仓仍优先，先命中先赢，不改既有语义。
  const sourceByProject = new Map();
  for (const entry of registry.entries) {
    if (entry && typeof entry.project === 'string' && typeof entry.sourcePath === 'string' && entry.sourcePath.trim() !== '') {
      const sp = path.resolve(entry.sourcePath);
      if (fs.existsSync(sp)) sourceByProject.set(entry.project, sp);
    }
  }
  if (shaRows.length > 0) {
    const gitRoot = resolveGitRoot(root);
    if (!gitRoot && sourceByProject.size === 0) {
      extraUnchecked.push('SHA 存在性未验：无可用 git 根（图谱目录非 git 仓且未设 ATLAS_GIT_ROOT）');
    } else {
      for (const row of shaRows) {
        const roots = [];
        if (gitRoot) roots.push({ label: gitRoot, dir: gitRoot });
        const sp = sourceByProject.get(row.project);
        if (sp && sp !== gitRoot) roots.push({ label: sp + '（源仓）', dir: sp });
        let matched = false;
        let anyMissing = false;
        let failMsg = null;
        for (const rt of roots) {
          const verdict = shaIsCommit(rt.dir, row.sha);
          if (verdict.ok) { matched = true; break; }
          if (verdict.missing) anyMissing = true;
          else if (verdict.fail) failMsg = verdict.fail;
        }
        if (matched) continue;
        if (anyMissing) {
          d.push(diag('p5-sha-broken', 'error', row.at, 'git SHA 不在仓中（' + row.sha + '，根：' + roots.map((r) => r.label).join(' / ') + '）——提交级事实声称失效', ['核对 SHA，或设 ATLAS_GIT_ROOT 指向含该提交的 git 仓，或在 state/projects.json 登记该项目 sourcePath']));
        } else {
          extraUnchecked.push('SHA 存在性未验：git 调用失败（' + failMsg + '）');
