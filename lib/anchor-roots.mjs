...
import { parseLocator } from './evidence.mjs';
...
// 目标不存在（写边不校验存在）或不可解析时退回 normalize 结果，不改变既有判定。
...
function underAnyRoot(roots, file) {
  ...
  return roots.some((r) => under(r.root, file) && under(r.root, real));
}
...
export function loadAnchorRootContext(sidecarPath, { allowRoots = [] } = {}) {
  ...
  for (const c of candidates) {
    ...
    seen.add(norm);
    roots.push({ root: norm, source: c.source });
  }
...
export function anchorRootVerdict(locator, context, exemptions, { cwd = process.cwd() } = {}) {
  ...
  const real = realFileOf(file);
  const hit = context.roots.find((r) => under(r.root, file) && under(r.root, real));
