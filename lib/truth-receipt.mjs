...
// 机器只校验回执为现存普通文件，不校验语义（生效与否属负责人）；set/transition 两入口共用。
...
export function checkTruthReceipt({ axis, from, to, receipt }) {
  const receiptFrom = from === undefined || from === null ? 'candidate' : from;
  if (axis !== 'truth' || !isTruthAdvance(receiptFrom, to)) {
  ...
  try {
    if (!fs.statSync(abs).isFile()) {
      return { ok: false, diagnostics: [diag('receipt_not_file', '回执必须是普通文件：' + abs, abs)] };
    }
  } catch (e) {
    const missing = e.code === 'ENOENT' || e.code === 'ENOTDIR';
    return { ok: false, diagnostics: [diag(missing ? 'receipt_not_found' : 'receipt_unreadable',
      (missing ? '回执文件不存在：' : '无法检查回执文件：') + abs + '（' + e.message + '）', abs)] };
  }
