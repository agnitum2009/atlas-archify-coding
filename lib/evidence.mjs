...
export function headAnchorState(locator, root, cache) {
  ...
  if (!entry) {
    ...
    if (show.error || show.status === null) {
      // git 可执行文件不可用/被信号终止：无法取得 HEAD 证据 ≠ 未提交。归免检（no-git）并注明原因，不误红。
      entry = { unavailable: true, reason: show.error ? String(show.error.code || show.error.message) : 'signal:' + show.signal };
