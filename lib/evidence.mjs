...
// git 子进程统一入口（2026-09-18 裁定）：缺省 10s 超时，ATLAS_GIT_TIMEOUT_MS 覆盖。超时 = 该项无法取得证据
// （headAnchorState 记 timeout 归「未检查」，其余调用点按既有失败路径处理），绝不让主命令挂死。
function gitTimeoutMs() {
  const v = Number(process.env.ATLAS_GIT_TIMEOUT_MS);
  return Number.isFinite(v) && v > 0 ? v : 10000;
}
export function gitSync(args, opts = {}) {
  return spawnSync('git', args, { encoding: 'utf8', timeout: gitTimeoutMs(), ...opts });
}
...
export function headAnchorState(locator, root, cache) {
  ...
  if (!repoRoot) return { state: 'no-git', file: abs, reason: 'no-repo' };
  const rel = path.relative(repoRoot, abs);
  const key = repoRoot + '\0' + rel;
  let entry = cache ? cache.get(key) : null;
  if (!entry) {
    const show = gitSync(['-C', repoRoot, 'show', 'HEAD:' + rel], { maxBuffer: HEAD_SHOW_MAXBUFFER });
    if (show.error && show.error.code === 'ETIMEDOUT') {
      entry = { timeout: true };
    } else if (show.error && (show.error.code === 'ENOENT' || show.error.code === 'EACCES')) {
      // git 可执行文件不存在/不可执行：无法取得 HEAD 证据 ≠ 未提交。归免检（no-git）并注明原因，不误红。
      // 只认这两类 spawn 失败；ENOBUFS/信号终止等仍走下方 fail-closed 路径（复审 2026-09-18）。
      entry = { unavailable: true, reason: show.error.code };
      ...
      const ig = gitSync(['-C', repoRoot, 'check-ignore', '-q', '--', rel]);
      // check-ignore 超时同样归「未检查」：不得让 status=null 落成 ignored=false → uncommitted 误红（复审 2026-09-18）。
      entry = ig.error && ig.error.code === 'ETIMEDOUT' ? { timeout: true } : { missing: true, ignored: ig.status === 0 };
    }
    if (cache) cache.set(key, entry);
  }
  if (entry.timeout) return { state: 'timeout', file: abs, repoRoot, rel };
  if (entry.unavailable) return { state: 'no-git', file: abs, repoRoot, rel, reason: 'git-unavailable:' + entry.reason };
...
export function checkVerifiedHeadAnchors(nodes, root, { sampleLimit = 5 } = {}) {
  const cache = new Map();
  const counts = { ok: 0, 'content-mismatch': 0, 'line-out-of-bounds': 0, uncommitted: 0, gitignored: 0, 'no-git': 0, broken: 0, unparseable: 0, timeout: 0 };
  ...
  for (const [id, node] of Object.entries(nodes || {})) {
    ...
    for (const locator of node.evidence || []) {
      ...
      else if (st.state === 'no-git') { noGit = true; noGitReasons[st.reason] = (noGitReasons[st.reason] || 0) + 1; }
    }
  ...
  const uncheckedCount = (counts.broken || 0) + (counts.unparseable || 0) + (counts.timeout || 0);
  let verdict;
  ...
  return {
    ...
    unchecked: { broken: counts.broken || 0, unparseable: counts.unparseable || 0, timeout: counts.timeout || 0 },
    noGitReasons,
    counts,
    errors,
    warnings,
