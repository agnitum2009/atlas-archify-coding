...
function assertSegment(value, kind) {
  if (typeof value !== 'string' || value.length === 0 || value === '.' || value === '..' || /[\\/\x00-\x1f]/.test(value) || path.isAbsolute(value)) {
    const err = new Error(kind + ' 须为非空单文件名/目录段');
    err.code = 'bad_args';
    throw err;
  }
}
...
function assertOutputWithin(root, target) {
  ...
  try {
    if (!within(root, target)) throw new Error('output escapes atlas root');
    // Resolve the root even before it exists; new children inherit its nearest existing ancestor.
    const rootParent = existing(root);
    const realRoot = path.resolve(fs.realpathSync(rootParent), path.relative(rootParent, root));
    const targetParent = existing(target);
    const realTarget = path.resolve(fs.realpathSync(targetParent), path.relative(targetParent, target));
    if (!within(realRoot, realTarget)) throw new Error('output escapes atlas root through symlink');
  } catch (err) { err.code = 'bad_args'; throw err; }
}
...
export function scaffoldAtlas(dir, { title, diagramType, diagramId, template }) {
  ...
  assertSegment(effectiveDiagramId, 'diagram-id');
  const project = deriveProject(root, diagramId);
  assertSegment(project, 'project');
  const outputs = [root, ...DIRS.map(sub => path.join(root, sub)),
    ...PROJECT_ZONES.map(zone => path.join(root, zone, project)),
    path.join(root, 'INDEX.md'), path.join(root, 'spec', project, effectiveDiagramId + '.json'),
    stateFile, path.join(root, 'state', 'projects.json'), path.join(root, 'rulings', 'RULINGS.md')];
  if (template === 'demo') outputs.push(path.join(root, 'spec', project, 'demo-map.json'));
  for (const target of outputs) assertOutputWithin(root, target);
  const created = [];
