...
import { findNode } from './store.mjs';
...
export function addTrace(sidecar, entry) {
  ...
  const node = entry.node ? findNode(sidecar, entry.node) : null;
  if (entry.node && !node) {
    const err = new Error('节点不存在：' + entry.node);
  ...
  if (node) {
    node.traceRefs = node.traceRefs || [];
    node.traceRefs.push(event.id);
  }
  return event;
}
...
export function replayNode(sidecar, nodeId, sinceIso) {
  const node = findNode(sidecar, nodeId);
  if (!node) return null;
  const events = [];
  ...
  events.sort((a, b) => compareEventTime(a.at, b.at));
  return {
    node: nodeId,
...
// 合法时间按毫秒排序且先于未知时间；未知值之间按字符串序，避免混合比较不传递。
export function compareEventTime(a, b) {
  ...
  if (validA) return ta < tb ? -1 : ta > tb ? 1 : 0;
  const sa = String(a);
  const sb = String(b);
