...
// progress × ledger 组合表（ADD-SPEC §2.4.1，2026-09-18 裁定：只成文既有隐含约束，不新立）。
// settled ⇒ progress=verified（settle/import 双写不变量）；cancelled ⇒ ledger=clean（取消的工作不可销账，欠账成孤儿）。
// 返回 null = 表内；否则返回违反的约束名（report 发 warning，不阻断）。
export function crossAxisUnlisted(progress, ledger) {
  if (ledger === 'settled' && progress !== 'verified') return 'settled_requires_verified';
  if (progress === 'cancelled' && ledger !== 'clean') return 'cancelled_requires_clean';
  return null;
}
...
export function validateTransition(axis, from, to) {
  if (!isAxis(axis)) {
    return { ok: false, diagnostics: [diag('unknown_axis', 'axis 必须是 truth|progress|ledger|class', axis + ':' + from + '->' + to)] };
  }
