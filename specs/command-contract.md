...
| cancelled_requires_clean | state set/transition（A2 §2.4.1，0.21.1）：progress→cancelled 时 ledger≠clean——取消的欠账成永久孤儿（settle 要求 in_progress/verified，import 要求 planned×clean，此后无事件可销）；真实账本统计该半边存量=0，零误伤收口 | 1 | 补救 = 先 state settle 核销欠账再取消；确属显式核销加 --correction（history corrected:true） |
