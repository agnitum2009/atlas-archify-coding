...
| cross_axis_unlisted | report（0.21.0，常开不依赖 --spec）：progress×ledger 组合不在 ADD-SPEC §2.4.1 表内（settled 而 progress≠verified / cancelled 而 ledger≠clean） | 不阻断（warning） | 补救 = state set --correction 修正一轴，或经 settle/import 事件；cancelled⇒clean 半边已由写边 cancelled_requires_clean 拦截（0.21.1），本码为读方兜底观测（含 --correction 留痕核销后的表外组合） |
| cancelled_requires_clean | state set/transition（A2 §2.4.1，0.21.1；0.21.2 起组合判定）：写入后组合为 progress=cancelled × ledger≠clean 即拒（双方向：progress→cancelled 带欠账，或 cancelled 节点 ledger 反向挂账；只拦 progress/ledger 成员轴写入，truth/class 等无关轴补记不冻结存量孤儿）——取消的欠账成永久孤儿（settle 要求 in_progress/verified，import 要求 planned×clean，此后无事件可销） | 1 | 补救 = 先 state set --axis ledger --value clean --correction 核销欠账（留痕）再取消；直接 --correction 放行则孤儿照落、读边持续告警（不推荐） |
